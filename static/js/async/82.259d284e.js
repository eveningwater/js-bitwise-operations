...
console.log(parseInt(binary, 2)); // \u{8F93}\u{51FA}: 10
  ...
  return (a & b) + ((a ^ b) >> 1);
}`,testCases:[{input:"10, 20",expected:15},{input:"5, 6",expected:5},{input:"0, 0",expected:0},{input:"-5, 5",expected:0}]}];E.forEach(u=>{let F=u.solution.match(/function\s+\w+\s*\(.*?\)\s*{/);F&&F[0]&&(u.solutionTemplate=F[0]+"\n    //...这里实现代码\n}")})},223:function(u,F,e){e.d(F,{Gk:()=>A,bo:()=>n,ew:()=>o,qY:()=>t,xC:()=>p,xQ:()=>i});var E=e(770);let D="js-bitwise-operations-progress",B={completedLevels:[],currentLevel:1,lastVisit:new Date().toISOString()},n=(0,E.iH)(function(){try{let u=localStorage.getItem(D);if(u)return JSON.parse(u)}catch(u){console.error("Failed to load progress:",u)}return{...B}}());function C(){try{n.value.lastVisit=new Date().toISOString(),localStorage.setItem(D,JSON.stringify(n.value))}catch(u){console.error("Failed to save progress:",u)}}function t(u){n.value.completedLevels.includes(u)||(n.value.completedLevels.push(u),C())}function p(u){n.value.currentLevel=u,C()}function o(u){return n.value.completedLevels.includes(u)}function A(){return[...n.value.completedLevels]}function i(){n.value={...B},C()}}}]);